...
func (c *Client) writePump() {
	...
	for {
		...
		case message, ok := <-c.send:
			...
			hours, minutes, _ := time.Now().Clock()
			message1 := []byte(fmt.Sprintf("%d:%02d - %s", hours, minutes, message))

			w.Write(message1)

			data := models.Message{
				HubName: c.hub.name,
				Data:    string(message1),
				Date:    time.Now(),
			}
			...
			if v, ok := paramsMap[stockKey]; ok {
				mapMutex.RUnlock()
				message := broker.ClientMessage{HubName: c.hub.name, ClientRemoteAddress: c.conn.RemoteAddr().String(), Message: v}
